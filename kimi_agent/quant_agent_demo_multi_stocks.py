...
class DataAgent:
    ...
    def run(self, stock_codes: List[str], start_date: str, end_date: str) -> Dict:
        """执行数据获取Pipeline"""
        print(f"\n{'='*50}")
        ...
        reports_map: Dict[str, List[ResearchReport]] = {}
        sentiment_results_map: Dict[str, List[Dict]] = {}
        ...
        for stock_code in stock_codes:
            ...
            stock_dataset[stock_code] = stock_data

            # 2. 获取研报数据
            reports = self.mcp.get_research_reports(stock_code, n=3)
            reports_map[stock_code] = reports
            ...
            for report in reports:
                ...
                print(f"    📊 [{stock_code}] {result['summary']}")
            sentiment_results_map[stock_code] = sentiment_results

            # 4. 构建情绪得分时序
            sentiment_scores = self._build_sentiment_series(
                stock_data.dates, reports, sentiment_results
            )
            sentiment_scores_map[stock_code] = sentiment_scores

        # 5. 构建等权组合数据 (保持后续流程最小改动)
        portfolio_data = self._build_portfolio_data(stock_dataset)
        sentiment_scores = self._build_portfolio_sentiment(
            portfolio_data.dates, sentiment_scores_map
        )

        print(
            f"✅ [{self.name}] 数据采集完成: {len(stock_dataset)}只股票, "
            f"{len(portfolio_data.prices)}条组合价格, {len(all_reports)}篇研报"
        )
        ...
        return {
            ...
            'sentiment_results_map': sentiment_results_map,
            'sentiment_scores': sentiment_scores,
            'sentiment_scores_map': sentiment_scores_map
        }
    ...
    def _build_portfolio_data(self, stock_dataset: Dict[str, StockData]) -> StockData:
        """构建等权组合行情数据"""
        prices_df = pd.concat(
            [s.prices.rename(code) for code, s in stock_dataset.items()],
            axis=1,
            join='inner'
        ).sort_index()
        volumes_df = pd.concat(
            [s.volumes.rename(code) for code, s in stock_dataset.items()],
            axis=1,
            join='inner'
        ).sort_index()
        ...
        return StockData(
            code='PORTFOLIO_EQ',
            prices=prices_df.mean(axis=1),
            volumes=volumes_df.sum(axis=1),
            dates=prices_df.index
        )

    def _build_portfolio_sentiment(self, dates: pd.DatetimeIndex,
                                   sentiment_scores_map: Dict[str, pd.Series]) -> pd.Series:
        """聚合多标的情绪为组合情绪"""
        aligned = [
            scores.reindex(dates).fillna(0).rename(code)
            for code, scores in sentiment_scores_map.items()
        ]
        if not aligned:
            return pd.Series(0.0, index=dates)
        sentiment_df = pd.concat(aligned, axis=1)
...
class FactorMiningAgent:
    ...
    def run(self, stock_dataset: Dict[str, StockData],
            sentiment_scores_map: Dict[str, pd.Series],
        ...
        for i in range(max_factors):
            ...
            factor_idea = self.llm.generate_factor(
                anchor_stock, [], existing_names + [f.name for f in factors]
            )
            print(f"    💡 创意: {factor_idea['description']}")
            print(f"    📝 解释: {factor_idea['explanation'][:80]}...")

            # 2. 在多标的上执行并评估因子，取平均IC与组合因子值
            per_stock_factor_values: List[pd.Series] = []
            per_stock_ic: List[float] = []
            for code, stock_data in stock_dataset.items():
                ...
                if factor_values is None or factor_values.isna().all():
                    continue
                per_stock_factor_values.append(factor_values.rename(code))
                per_stock_ic.append(self._calculate_ic(factor_values, stock_data.returns()))

            if not per_stock_factor_values:
                print(f"    ❌ 因子在多标的上执行失败，跳过")
                continue

            factor_matrix = pd.concat(per_stock_factor_values, axis=1, join='inner')
            factor_values = factor_matrix.mean(axis=1)
            ic = float(np.mean(per_stock_ic)) if per_stock_ic else 0.0
    ...
    def _execute_factor_expression(self, expr: str, stock_data: StockData,
        ...
        try:
            ...
            # lambda 会在 eval 的全局命名空间解析符号，因此常用函数放在 globals
            safe_globals = {"__builtins__": {}, 'np': np, 'pd': pd, 'abs': abs}
...
def main():
    ...
    # 运行投研Pipeline
    results = system.run(
        stock_codes=['600519.SH', '000001.SZ', '300750.SZ'],  # 模拟多标的
